...
func deployEtcdCluster(v *api.Cluster) (*eopapi.EtcdCluster, error) {
	ec := &eopapi.EtcdCluster{
		...
		ObjectMeta: metav1.ObjectMeta{
			Name:      etcdNameForKinky(v.Name),
			Namespace: v.Namespace,
			Labels:    labelsForKinky(v.Name),
		},
...
// etcdNameForKinky returns the etcd cluster's name for the given vault's name
func etcdNameForKinky(name string) string {
	return name + "-etcd"
}

// etcdURLForKinky returns the URL to talk to etcd cluster for the given vault's name
func etcdURLForKinky(name string) string {
	return fmt.Sprintf("https://%s-client:2379", etcdNameForKinky(name))
}
